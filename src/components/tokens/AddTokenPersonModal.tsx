...
export function AddTokenPersonModal({ open, onOpenChange, onSubmit }: Props) {
  const { direction } = useLanguage();
  const isHe = direction === 'rtl';
  const [form, setForm] = useState({ full_name: '', id_number: '', company: '', phone_number: '', token_type: '' });

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.full_name.trim() || !form.id_number.trim() || !form.company.trim() || !form.phone_number.trim() || !form.token_type.trim()) return;
    onSubmit(form);
    setForm({ full_name: '', id_number: '', company: '', phone_number: '', token_type: '' });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent dir={direction} className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{isHe ? 'הוספת איש חדש' : 'Add New Person'}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit">{isHe ? 'הוסף' : 'Add'}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
